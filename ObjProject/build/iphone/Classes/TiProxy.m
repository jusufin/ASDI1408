...
//Rare exceptions to indicate a bug in the _objproject code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_objproject = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_objproject]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.objproject.module"
